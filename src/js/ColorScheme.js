...
class ColorScheme {
	...
	/**
	 * A color scheme is a list of Color objects
	...
	constructor(colorList) {
		...
		this.colors.sort(ColorScheme.compareColors);
	}
	...
	static generateMonochromatic(seedHue) {
		return new ColorScheme([new Color({
			'hue': seedHue,
			'saturation': ColorScheme.generateRandomInteger(60, 100),
			'value': ColorScheme.generateRandomInteger(60, 85),
		})]);
	}
	...
	static generateAnalogous(seedHue) {
		...
		let saturations = ColorScheme.shuffleArray([
			ColorScheme.generateRandomInteger(50, 100),
			ColorScheme.generateRandomInteger(30, 70),
			ColorScheme.generateRandomInteger(30, 70),
		]);
		...
		for (i = 0; i < 3; i++) {
			list.push(new Color({
				...
				'value': ColorScheme.generateRandomInteger(40, 60),
			}));
		}

		return new ColorScheme(list);
	}
	...
	static generateComplementary(seedHue) {
		return new ColorScheme([
			new Color({
				'hue': seedHue,
				'saturation': ColorScheme.generateRandomInteger(50, 100),
				'value': ColorScheme.generateRandomInteger(30, 70),
			}),
			new Color({
				'hue': seedHue + 180,
				'saturation': ColorScheme.generateRandomInteger(20, 70),
				'value': ColorScheme.generateRandomInteger(30, 70),
			}),
		]);
	}
	...
	static generateSquareTetradic(seedHue) {
		let saturations = ColorScheme.shuffleArray([
			ColorScheme.generateRandomInteger(50, 100),
			ColorScheme.generateRandomInteger(50, 70),
			ColorScheme.generateRandomInteger(30, 70),
			ColorScheme.generateRandomInteger(30, 60),
		]);
	...
	static compareColors(a, b) {
		return (
			ColorScheme.compareNumeric(a.getHue(), b.getHue()) ||
			ColorScheme.compareNumeric(a.getSaturation(), b.getSaturation()) ||
			ColorScheme.compareNumeric(a.getValue(), b.getValue())
		) || 0;
	}
	...
	static shuffleArray(array) {
		...
		for (i = ilen - 1; i >= 1; i--) {
			j = ColorScheme.generateRandomInteger(0, i);
			if (i === j) {
				continue;
			}
			tmp = array[i];
	 ...
	 * This can give you more information about the type of color scheme,
	...
	getRelativeHueList() {
		...
		for (i = 0; i < len; i++) {
			for (j = i + 1; j < len; j++) {
				list.push(ColorScheme.getAngularDelta(
					hues[i],
					hues[j]
				));
			}
		...
		return list.sort(ColorScheme.compareNumeric);
	}
